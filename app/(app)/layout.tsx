    ...
    <>
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur">
        <div className="section flex items-center justify-between py-3">
          <Link href="/app" className="font-display text-lg font-bold text-marine-900">
            trycut
          </Link>
          <div className="flex items-center gap-3">
            <span className="rounded-full bg-marine-50 px-3 py-1 text-sm font-semibold text-marine-600">
              {session.profile.credits_remaining} coupe
              {session.profile.credits_remaining > 1 ? 's' : ''}
            </span>
            {session.profile.is_admin ? (
              <Link href="/admin" className="text-sm font-semibold text-marine-600 underline">
                Inscrits
              </Link>
            ) : null}
            <Link href="/compte" className="text-sm font-semibold text-marine-600 underline">
              Compte
            </Link>
